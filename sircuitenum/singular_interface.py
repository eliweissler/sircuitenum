...
def solve_with_minass_grobcov(equations):
    """
    Solves a system of SymPy equations using minAssGTZ -> indepSet -> grobcov.
    
    This approach doesn't require specifying which variables are parameters vs unknowns.
    It automatically discovers the structure by:
    1. Computing minimal associated primes (irreducible components)
    2. For each component, finding the maximal independent set (free variables)
    3. Running grobcov with those as parameters to get the full stratification
    
    Args:
        equations (list): List of SymPy expressions (assumed equal to 0).

    Returns:
        list[dict]: A list of branches with constraints and solutions.
    """
    # Collect all symbols
    all_symbols = set()
    for eq in equations:
    ...
    all_vars = sorted(list(all_symbols), key=str)
    str_vars = ", ".join(str(v) for v in all_vars)
    ...
    all_var_names = [str(v) for v in all_vars]
    parsed_results = []
    ...
    for b_text in raw_branches:
        ...
        for line in lines[1:]:
            ...
            if "|||PARAMS|||" in line:
                ...
                branch_data['params'] = [v.strip() for v in params_str.split(",") if v.strip()]
                continue
            if "|||VARS|||" in line:
...
if __name__ == "__main__":
    ...
    # Test 3: minAssGTZ workflow - same system without specifying solve_vars
    print("\n=== Test 3: minAssGTZ workflow (auto-discover structure) ===")
    eqs = [a*x - b]
    branches = solve_with_minass_grobcov(eqs)
    print(f"Found {len(branches)} Branches:")
    for br in branches:
        print(f"  Branch {br['id']}: component={br['component']}, params={br['params']}, vars={br['vars']}")
